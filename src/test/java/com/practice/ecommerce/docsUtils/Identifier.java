package com.practice.ecommerce.docsUtils;

public class Identifier {

	public static final String STORE_OWNER_SIGN_UP_201_OK = "store-owner register-201";
	public static final String STORE_OWNER_SIGN_UP_400_BAD = "store-owner register-400";
	public static final String STORE_OWNER_SIGN_IN_200_OK = "store-owner sign-in-200";
	public static final String STORE_OWNER_SIGN_IN_400_BAD = "store-owner sign-in-400";
	public static final String STORE_OWNER_MY_INFO_200_OK = "store-owner my-info-200";
	public static final String STORE_OWNER_MY_INFO_401_UNAUTHORIZED = "store-owner my-info-401";
	public static final String STORE_OWNER_LOGOUT_200_OK = "store-owner logout-200";
	public static final String STORE_OWNER_UPDATE_PASSWORD_200_OK = "store-owner update-password-200";
	public static final String STORE_OWNER_UPDATE_PASSWORD_400_BAD = "store-owner update-password-400";
	public static final String STORE_OWNER_UPDATE_PASSWORD_401_UNAUTHORIZED = "store-owner update-password-401";
	public static final String STORE_OWNER_DELETE_200_OK = "store-owner delete-200";
	public static final String STORE_OWNER_DELETE_401_UNAUTHORIZED = "store-owner delete-401";
}
